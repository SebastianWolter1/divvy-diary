...
const Cta = ({ type }) => {
  ...
  const buttonText = type === 'login' ? 'Already have an account?' : type === 'register' ? "Don't have an account?" : type === 'logout' ? 'Logout' : type === 'loginForm' ? 'Login' : type === 'registerForm' ? 'Register' : null ;
